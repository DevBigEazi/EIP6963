...
function App() {
  ...
  const handleSwitchChain = async () => {
    try {
      ...
      const provider = injectedProviders.get(connection.providerUUID)!.provider;
        ...
        <Flex py="4" className="flex-col md:flex-row gap-4 p-4 md:p-0">
          <Box className="w-full md:w-1/2">
            <Flex align={"center"} className="gap-2 mb-4">
              <Heading as="h2">Available Injected wallets</Heading>
              <RocketIcon width={24} height={24} />
            </Flex>
            {injectedProviders.size === 0 ? (
              <div>
                You do not have any wallet extension installed on your browser
              </div>
            ) : (
                ...
                <Flex className="flex-col gap-2">
                  <Flex className="gap-2">
                    <span>Connected to:</span>
                    {
                      <Flex gap="1" align="center">
                        <span>
                          {}
                          {connectedInjectectProvider.info.name}
                        </span>{" "}
                        <img
                          className="w-5 h-5 rounded"
                          src={connectedInjectectProvider.info.icon}
                          alt={connectedInjectectProvider.info.name}
                        />
                      </Flex>
                    }
                  </Flex>
                  <Flex className="gap-2">
                    ...
                    <Flex gap="1" align="center">
                      ...
                      {isSupportedChain(connection?.chainId) ? (
                        <Badge color="green">Supported</Badge>
                      ) : (
                        <Badge color="orange">Unsupported</Badge>
                      )}
                    </Flex>
                  </Flex>
                  <Flex className="gap-2">
